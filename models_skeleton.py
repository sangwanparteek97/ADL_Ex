...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        ...
        self.output_linear = nn.Linear(dim_head*heads,dim)
        self.followed_droput = nn.Dropout(dropout)
        # TODO
    ...
    def forward(self, x, context = None, kv_include_self = False):
        ...
        # TODO: attention
            #### first linear multiplication to find wx
        q = self.q(x)
        k = self.k(x)
        v = self.v(x)
        #### attention
        dots_product = torch.matmul(q, k.transpose(2, 3)) / torch.sqrt(torch.tensor(self.dim_head))
        attention = self.softmax(dots_product)
...
class ProjectInOut(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        # TODO
        ### projection into input space
        x = self.project_in(x)
        x = self.fn(x) ## function that transforms data into new dim
        x = self.project_out(x) ### output dimention
        return x
